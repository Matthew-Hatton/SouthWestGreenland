...
scaling <- seq(0,3.99,0.01) #define scale factors from focal gear file
files <- list.files() #lists files - need to set wd first
files2 <- mixedsort(sort(files)) #sort the files
master <- read.csv("..\\..\\mastercombinationsdf.csv") %>% 
...
finaldiscards <- data.frame() #empty df
...
for(i in 1:length(investigating)){
  ...
  finaldiscards <- rbind(finaldiscards,result) #bind all together
  print(paste0(i,"/",length(investigating))) #progress
}
row.names(finaldiscards) <- NULL #reset rownames

finaldiscards$Focal.Gear<-gsub(".rds","",as.character(finaldiscards$Focal.Gear)) #fix gearnames
toc()
cols <- colnames(finaldiscards)[2:13]
finaldiscardsgears <- pivot_longer(data = finaldiscards,cols = cols,
                      names_to = "Gear",
                      values_to = "Catch")
