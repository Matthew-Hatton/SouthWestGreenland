...
Packages <- c("tidyverse", "sf", "stars", "rnaturalearth", "raster","ggspatial","ncdf4")        # List handy packages
...
setwd("C:\\Users\\psb22188\\Documents\\PhD\\22-23\\West Greenland\\NEMO\\Data Wrangling\\Jacks Way\\Objects")
Bathymetry <- readRDS("NA_grid.rds") %>%                          # Import NEMO-MEDUSA bathymetry
  st_drop_geometry() %>%                                                    # Drop sf geometry column 
  dplyr::select(-c("x", "y"), latitude = Latitude, longitude = Longitude) 
...
ggplot(clipped) +
  ...
  geom_sf(aes(fill = Depth), alpha = 0.5) +
  geom_point(data = scheme,aes(x = longitude,y = latitude),size = 0.1,colour = "black") + #check to see NEMO points
  scale_x_continuous(expand = c(0,0)) +
  scale_y_continuous(expand = c(0,0)) +
  theme(legend.position="none") +
  annotation_scale(location = "bl", width_hint = 0.5)
ggsave(filename = "NEMO points on map.pdf")
