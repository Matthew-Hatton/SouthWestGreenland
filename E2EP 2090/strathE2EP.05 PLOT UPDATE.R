...
new <- read.csv("Models/SW_Greenland/2090-2099/Driving/physics_SWG_2090-2099.csv") %>%   # Read in example boundary drivers
  select(SO_IceFree, SI_IceFree, SO_IceCover, SI_IceCover, 
         SO_IceThickness,SO_temp, D_temp, SI_temp, log10Kvert,
         SI_IceThickness,SO_SnowThickness,SI_SnowThickness) %>% 
  mutate(Month = 1:12) %>% 
...
facets <- c("SO_IceFree","SO_SnowThickness","SI_SnowThickness")
