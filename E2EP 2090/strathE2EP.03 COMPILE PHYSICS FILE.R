...
Physics_template <- read.csv("Models/SW_Greenland/2011-2019/Driving/physics_SWG_2011-2019.csv") # Read in example Physical drivers
Depths <- read.csv("Models/SW_Greenland/2011-2019/Param/physical_parameters_SWG.csv", nrows = 3)# Import Mike's original depths for scaling
...
My_V_Flows <- filter(My_V_Flows,between(Year, 2090, 2099)) %>%                                     # Limit to reference period
  group_by(Month) %>% 
...
#fix model breaking - load in 2010s physics file
physics_2010 <- read.csv("./Models/SW_Greenland/2011-2019/Driving/physics_SWG_2011-2019.csv")
#replace chunk
Physics_new[7:9,26:31] <- physics_2010[7:9,26:31]
#replace individual NaN's
Physics_new$SO_IceCover[10] <- physics_2010$SO_IceCover[10]
Physics_new$SI_IceCover[6] <- physics_2010$SI_IceCover[6]
Physics_new$SO_IceThickness[10] <- physics_2010$SO_IceThickness[10]
Physics_new$SI_IceThickness[6] <- physics_2010$SI_IceThickness[6]
Physics_new$SO_SnowThickness[10] <- physics_2010$SO_SnowThickness[10]
Physics_new$SI_SnowThickness[6] <- physics_2010$SI_SnowThickness[6]
write.csv(Physics_new, file = "Models/SW_Greenland/2090-2099/Driving/physics_SWG_2090-2099.csv", row.names = F)


sed <- read.csv("Models/SW_Greenland/2011-2019/Param/physical_parameters_SWG.csv")
...
write.csv(sed, file = "Models/SW_Greenland/2090-2099/Param/physical_parameters_SWG.csv", row.names = F)
